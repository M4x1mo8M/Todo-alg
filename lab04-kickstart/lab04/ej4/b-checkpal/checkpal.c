...
int main(void) {
    ...
    for (unsigned int i=0; i < SIZEOF_ARRAY(ignore_chars); i++) {
        ...
        free(filtered);  // Liberamos la memoria anterior
        filtered = temp;  // Actualizamos el puntero
    }
    ...
    free(filtered);  // Liberamos la memoria asignada a filtered
    filtered = NULL; // Evitamos el uso de punteros colgantes
    return EXIT_SUCCESS;
}
